...
pair<double*, vector<int>> centerpoint(int node, double center[3], int voxelsize) {
...
// Define a custom hash function for double[3] pointed to by double*
 struct DoubleArrayPointerHash {
	std::size_t operator()(const double* ptr) const {
		// Convert the double* pointer to double[3] and hash its content
		const double* arr = ptr;
		std::size_t seed = 0;
		for (int i = 0; i < 3; ++i) {
			seed ^= std::hash<double>{}(arr[i]) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
		}
		return seed;
	}
};

// Define a custom equality comparator for double* pointers based on the content of double[3]
struct DoubleArrayPointerEqual {
	bool operator()( const double* ptr1,const  double* ptr2) const {
		// Compare the content of double[3] pointed to by the double* pointers
		const double* arr1 = ptr1;
		const double* arr2 = ptr2;
...
//std::unordered_set<double*, DoubleArrayPointerHash, DoubleArrayPointerEqual> grpvisited;


//grp size 4 join center
//problem in find here
pair<double*, vector<double*>> grpcenterpoint(double node[3], int voxelsize, int depth, std::unordered_set<double*, DoubleArrayPointerHash, DoubleArrayPointerEqual> &grpvisited) {
	...
	for (int i = 0; i < grppoints.size(); i++) {
		...
		// check here
		if (grpvisited.find(cord) == grpvisited.end()) {
			cout << "runned " <<i<< endl;
...
// 
	...
	int level;
	cout << "Enter the level of tree support structure" << endl;
	cin >> level;
	...
	*/
	...
	while (++depth <= level) {
		...
		for (int i = 0; i < grppoints.size(); i++) {
			...
			if (grpvisited.find(grppoints[i]) == grpvisited.end()) {
				cout << "test1 ====================================="<<i << endl;
				vtkSmartPointer<vtkPoints> linepoints = vtkSmartPointer<vtkPoints>::New();

				double node[3] = { grppoints[i][0],grppoints[i][1],grppoints[i][2] };
				auto x = grpcenterpoint(node, voxelsize, depth ,grpvisited);

				auto joincenter = x.first;
				auto joinee = x.second;

				double joincenterdata[3] = { joincenter[0],joincenter[1],joincenter[2] };
				linepoints->InsertNextPoint(joincenterdata); // main point 0
				...
				for (int i = 0; i < joinee.size(); i++) {
					...
					vtkIdType lineIndices[2] = { 0, i }; // Connect Point i to Point 0
					cells->InsertNextCell(2, lineIndices);
				}
